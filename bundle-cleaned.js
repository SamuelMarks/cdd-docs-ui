  ...
  if (schema2.type === "object" && schema2.properties) {
    ...
    for (const [name, prop] of Object.entries(schema2.properties)) {
      ...
      html += `<tr id="${propId}" class="cdd-deep-link-row">
        ...
        <td><span class="cdd-prop-desc">${renderMarkdown(propSchema.description)}</span></td>
      </tr>`;
    ...
    if (schema2.description) html += `<p class="cdd-prop-desc">${renderMarkdown(schema2.description)}</p>`;
  }
  html += `</div>`;
  return html;
}
  ...
  <div class="cdd-layout">
    ...
    <main class="cdd-main">
      <header>
        ...
        <p>${renderMarkdown(spec.info.description)}</p>
